...
extern g_start_flag;
extern g_suspend_flag;

/*隔5秒执行一次，向服务器上报数据（歌曲 音量 模式 设备号）
函数返回值：无
函数参数：信号值
*/
...
void socket_recv_data(char *msg)
{
	...
	len = *(int *)buf;//将发送过来的数据长度值从buf中强转存在len
	printf("-- LENGTH : %d\n", len);
    size = 0;
	memset(buf, 0, 1024);
	//获取数据内容
...
int init_socket()
{
	...
	while (count--)
	{
		...
		//连接成功
		FD_SET(g_sockfd,&READSET); //将网络文件标识符加入集合
		g_maxfd = (g_maxfd < g_sockfd) ? g_sockfd : g_maxfd;
		 
		//音响2秒上传一次 保活
		//alarm(2);            //SIGALRM
		signal(SIGALRM,send_server);   //进程收到信号 向服务器发送数据
		send_server(SIGALRM);

		break;
	}
}
/*每次更新歌曲链表，都向服务器发送数据，传送给app*/
void upload_music_list()
{
	...
	socket_send_data(obj);//发给服务器

	json_object_put(obj);//清空内存
